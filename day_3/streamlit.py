...
prompt = PromptTemplate(
    ...
    For each sentence, include a reference to the context using reference.title and reference.url if available.
    <|eot_id|><|start_header_id|>user<|end_header_id|>
    Question: {question}
    Context: {context}
...
#
    ...
    web_results = "\n".join([d["content"] + " (reference.title : " + d["title"] + ", refernce.url : " + d["url"] + ")" for d in docs])
...
nodes = ["websearch", "retrieve", "grade_documents", "generate", "END"]
...
edges = [("retrieve", "grade_documents"), ("grade_documents", "websearch"), ("grade_documents", "generate"), ("websearch", "grade_documents"), ("generate", "END"), ("generate", "websearch")]
G.add_edges_from(edges)

# 노드와 엣지의 리스트를 사이드바에 표시
...
inputs = {"question": f"{user_input}"}

if st.button("Start Process !! "):
    msg = ""
    for output in app.stream(inputs):
        for key, value in output.items():
            msg += f"Finished running: {key}:"

    msg += value["generation"]

    text_area = st.text_area("Result", value=msg, height=200)
